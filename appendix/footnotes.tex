...
% mettere a posto bibliografia con autore e titolo - ordine preferibilmente alfabetico
...
\end{demonstration}
La seguente dimostrazione sulle restrizioni di omeomorfismi è un'elaborazione personale sulle basi dei teoremi precedenti.
\begin{corollary}
Se $\funz{f}{X}{Y}$ è un omeomorfismo, allora $\funz{f_{\mid Z}}{Z}{f\left(Z\right)}$ è omeomorfismo per ogni sottospazio $Z\subseteq X$.
\end{corollary}
\begin{demonstration}
	La restrizione di una funzione è sempre una biezione e, per il teorema precedente, è anche continua. Ciò vale sia per $f$, sia per l'inversa $f^{-1}$; segue dunque la tesi.
\end{demonstration}
