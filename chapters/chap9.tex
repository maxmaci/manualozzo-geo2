...
Supponiamo che i vertici del poligono \textit{non} siano tutti equivalenti. Dunque, esisterà un lato $a$ i cui due vertici $P$ e $Q$ \textit{non} sono equivalenti!
\begin{center}
	\includegraphics[trim=0cm 0cm 0cm 0cm, clip, scale=0.35]{images/cutandpastealgorithmstep2-1.pdf}
\end{center}
Consideriamo il secondo lato $b$ con vertice $Q$ e, facendo un taglio come in figura, incolliamo il lato $a$:
\begin{center}
...
Supponiamo che ci sia una coppia di lati $b$ \textit{non} adiacenti del II tipo. Operiamo un taglio $c$ tra gli estremi finali della coppia e incolliamo la coppia stessa.
	\begin{center}
...
\begin{enumerate}
	\item[\underline{4.1}] Taglio un lato $c$ lungo gli estremi finali di $a$ e incollo $b$.
	\begin{center}
		\includegraphics[trim=0cm 0cm 0cm 0cm, clip, scale=0.35]{images/cutandpastealgorithmstep4-3.pdf}
	\end{center}
	\item[\underline{4.2}] Taglio un lato $d$ lungo gli estremi finali di $c$ e incollo $a$.
	\begin{center}
...
\end{demonstration}
\begin{observe}
	Si ottiene sempre un piano proiettiva se e solo nel modello iniziale c'è almeno una coppia di lati del II tipo (perché i passi mantengono il tipo).
\end{observe}
\section{Orientabilità}
\begin{define}\textsc{Nastro di Möbius}.\\
	\begin{minipage}{.75\linewidth}
Definiamo il \textbf{nastro di Möbius} (aperto) $N_0\subseteq \realset^3$ come lo spazio $\left[0,\ 1\right]\times (0,\ 1)$ ($\left[0,\ 1\right]\times [0,\ 1]$ nel caso chiuso $N$) con i lati quozientati come in figura.
	\end{minipage}
...
\end{define}
\begin{observe}
	$N_0$ è una superficie topologica, ma \textit{non} è compatta in quanto \textit{non} è chiuso.
	\vspace{-3mm}
\end{observe}
\begin{intuit}
	Immaginiamo di essere una \textit{figura bidimensionale} posta sulla sfera $S^2$; essa è libera di muoversi su di essa. Qualunque percorso chiuso essa faccia, la figura tornerà al punto di partenza \textit{esattamente come era partita}.\\
...
\end{equation}
In particolare, se poniamo $T_0=S^2$ come caso ‘‘degenere'' di toro non buco e $\euler{T_g}{\ }=2-2g\ \forall g\geq 0$:
\begin{itemize}
