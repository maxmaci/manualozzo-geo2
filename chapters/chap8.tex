...
%RICKROLL
	\subsection{Gruppo fondamentale sfere}
...
Vediamo ora un risultato generale per poter calcolare un gruppo fondamentale sfruttando proprio la nozione di generatore. Si determinerà il gruppo fondamentale $\pi_1(X,\ x_0)$ nel corso di \textit{Geometria 4}
\begin{theorema} \textsc{Van Kampfen su generatori, Manetti 11.25} \label{van kampfen} \\
	Sia $X$ uno spazio topologico e siano $A,B\subseteq X$ aperti tali che $A,B,A\cap B$ sono c.p.a e $A\cap B\neq\emptyset, X=A\cup B$ (per il teorema	 questo implica che $X$ è c.p.a.)
%AAA TEOREMA CERCASI
	Sia $x_0\in A\cap B$ e le inclusioni $\incl i A X$ e $\incl j B X$ con i loro morfismi indotti $\funz {i_*} {\pi_1(A,\ x_0)} {\pi_1(X,\ x_0)}$ e $\funz {j_*} {\pi_1(B,\ x_0)} {\pi_1(X,\ x_0)}$. Siano inoltre $G_A\coloneqq 	IM i_*$ e $G_B\coloneqq IM j_*$\newline
