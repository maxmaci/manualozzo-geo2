...
\begin{remember}{n}
	Sia $V$ spazio vettoriale su campo $\K$ di dimensione finita. Consideriamo gli \textbf{endomorfismi} di $V$ o, equivalentemente, le matrici $M_{n,n}\left(\K\right)$, dove $\dim V=n$.
	\begin{itemize}
		\item $A$ determina un endomorfismo di $\K^n$ dato da $v\mapsto A\mathbf{v}$.
		\item Matrici associate allo stesso endomorfismo rispetto a basi diverse sono \textbf{simili}\index{matrice!simile}, cioè esiste $P\in\GL\left(n\, \K\right)$ tale che $B=P^{-1}AP$.
		\item Le seguenti affermazioni sono equivalenti:
		\begin{itemize}
			\item $A$ è \textbf{diagonalizzabile}\index{diagonalizzazione}.
			\item $A$ è simile ad una matrice diagonale $D$ con $P$ matrice con \textbf{autovettori} di $A$ sulle colonne.
			\item $\K^n$ ammette una base di \textbf{autovettori}\index{autovettore} di $A$.
			\item $\K^n=V_{\lambda_1}\oplus\ldots\oplus V_{\lambda_r}$ con $V_{\lambda_i}$ \textbf{autospazio}\index{autospazio} relativo ad $A$
		\end{itemize}
	\end{itemize}
\end{remember}
\begin{definition}{}[Diagonalizzazione simultanea]
	Siano $A,\ B\in\K^{n, m}$ due matrici \textit{diagonalizzabili}. Diciamo che $A$ e $B$ sono \textbf{simultaneamente diagonalizzabili}\index{diagonalizzazione!simultanea} se esiste una base di $\K^n$ composta di autovettori contemporaneamente sia di $A$ sia di $B$. Equivalentemente, $A$ e $B$ sono simultaneamente diagonalizzabili se esiste una matrice invertibile $P$ tale che $P^{-1}AP$ e $P^{-1}BP$ sono \textit{entrambe diagonali}.
\end{definition}
\begin{example}{n}
	Non tutte le matrici diagonalizzabili lo sono simultaneamente. Prendiamo $\R^2$; si consideri:\\
...
\begin{theorem}{}[Caratterizzazione della diagonalizzazione simultanea]\label{teoremasimdiag}
Siano $A,\ B\in\K^{n, n}$. Allora $A$ e $B$ sono simultaneamente diagonalizzabili se e solo se $A$ e $ B$ sono diagonalizzabili e $A,\ B$ commutano, cioè $AB=BA$.
\end{theorem}
Per dimostrare il teorema, abbiamo tuttavia bisogno del seguente lemma:
\begin{lemma}{}[Matrici che commutano e autospazi]
	Siano $A,\ B\in\K^{n, n}$ tale che $AB=BA$ e sia $W$ un autospazio di $B$. Allora, presa l'azione di $\GL\left(n,\ \K\right)$ su $\K^{n, n}$, si ha che $A\ldotp W\subseteq W$.
\end{lemma}
\begin{proof}{n}
...
\end{equation*}
$A\ldotp \mathbf{w}$ è autovettore rispetto a $\lambda$, pertanto $A\ldotp \mathbf{w}\in W,$ per ogni $\mathbf{w}$ e dunque segue la tesi.\qedhere
\end{proof}
\begin{proof}{n}[del teorema \ref{teoremasimdiag}]~{}\\
	$\rightimplies$Per ipotesi, esiste $P\in \GL\left(n,\ \K\right)$ tale che $D_1=P^{-1}AP$ e $D_2=P^{-1}BP$ sono diagonali; in particolare commutano, in quanto matrici diagonali: $D_1D_2=D_2D_1$. Allora $A=PD_1P^{-1}$, $B=PD_2P^{-1}$ e
	\begin{equation*}
		AB=\left(PD_1P^{-1}\right)\left(PD_2P^{-1}\right)=PD_1D_2P^{-1}=PD_2D_1P^{-1}=\left(PD_2P^{-1}\right)\left(PD_1P^{-1}\right)=BA.
	\end{equation*}
$\leftimplies$Procediamo con una \textit{dimostrazione costruttiva}. Sappiamo che:
\begin{itemize}
	\item Poiché $A$ è diagonalizzabile esistono $\mathbf{v}_1,\ \ldots,\ \mathbf{v}_n$ base di $V$ composta da \textit{autovettori} di $A$;
	\item Poiché $B$ è diagonalizzabile si ha $V=W_1\oplus\ldots\oplus W_r$ con $W_j$ \textit{autospazi} di $B$.
\end{itemize}
Consideriamo $\mathbf{v}_1\in V$. Esso si scrive in modo unico come
\begin{equation*}
	\mathbf{v}_1=\mathbf{w}_{1,1}+\ldots+\mathbf{w}_{1,r}\text{ con }\mathbf{w}_{1,j}\in W_j,\ \forall j=1,\ \ldots,\ r.
\end{equation*}
...
\end{equation*}
In altre parole, $\forall j,\ \mathbf{w}_{1,j}$ è $\mathbf{0}$ oppure un autovettore di $A$ e, per ipotesi, anche di $B$. Procediamo allo stesso modo tutti i vettori della base $\mathbf{v}_1, \ldots,\ \mathbf{v}_n$: si ha che $\mathbf{v}_i=\mathbf{w}_{i,1}+\ldots+\mathbf{w}_{i,r}$ e $\forall i, j,\ $ $\mathbf{w}_{i,j}$ è $\mathbf{0}$ oppure un autovettore comune di $A$ e $B$. Otteniamo un insieme $\left\{\mathbf{w}_{i, j}\right\}$ di autovettori comuni di $A$ e $B$. Per costruzione, lo \textit{span lineare} dei $\left\{\mathbf{w}_{i, j}\right\}$ contiene $\left\{\mathbf{v}_1,\ \ldots,\ \mathbf{v}_n\right\}$ e pertanto è necessariamente pari a $V$! In altre parole, $\left\{\mathbf{w}_{i,\ j}\right\}$ è un sistema di generatori di $V$ e possiamo estrarre da esso una base di $V$ costituita di autovettori comuni ad $A$ e a $B$.
\end{proof}
\section{Polinomi e matrici}
\begin{remember}{n}
	Dato un campo $\K$, indichiamo con $\K\left[t\right]$ l'anello dei polinomi a coefficienti in $\K$ nella variabile $t$; un suo elemento $f\left(t\right)\in\K\left[t\right]$ è della forma:
	\begin{equation*}
		f\left(t\right)=b_nt^n+b_{n-1}t^{n-1}+\ldots+b_1t+b_0\text{ con }b_i\in\K
	\end{equation*}
\end{remember}
Finora abbiamo sempre \textit{valutato} i polinomi in valori del campo $\K$. Possiamo invece valutarli in una \textit{matrice} in $\K^{n, n}$? Dopotutto, la \textit{somma di matrici} e la \textit{moltiplicazione per uno scalare} sono operazioni \textit{interne} a $\K^{n, n}$ e pertanto potremmo pensare che sia lecito. Tuttavia, presi i polinomi $p$ così come sono, non sarebbe \textit{ben definita} $p\left(A\right)$ a causa del \textbf{termine noto}; infatti, non possiamo sommare uno \textit{scalare ad una matrice}! Per ovviare a questo problema, quando valutiamo un polinomio in una matrice $A$ ‘‘\textit{correggiamo}'' il termine noto con la \textbf{matrice identità} $I$:
\begin{equation*}
	f\left(A\right)\coloneqq b_nA^n+b_{n-1}A^{n-1}+\ldots+b_1A+b_0I\ \text{con}\ b_i\in\K
\end{equation*}
In questo modo, $f\left(A\right)\in\K^{n, n}$.
\begin{example}{n}
Preso $f\left(t\right)=t^2-3$, il polinomio valutato nella matrice $A$ è $f\left(A\right)=A^2-3I$.
\end{example}
\begin{remark}{n}
	Dati $f,\ g\in\K\left[t\right]$ e $A\in\K^{n,n}$, si ha:
	\begin{itemize}
...
\end{remark}
\begin{proof}{n}
	Prendiamo i polinomi
	\begin{align*}
		f\left(t\right)&=b_nt^n+b_{n-1}t^{n-1}+\ldots+b_1t+b_0 g\left(t\right)&=c_nt^n+c_{n-1}t^{n-1}+\ldots+c_1t+c_0
	\end{align*}
	e valutiamoli entrambi in $A$:
	\begin{align*}
		f\left(A\right)=b_nA^n+b_{n-1}A^{n-1}+\ldots+b_1A+b_0I g\left(A\right)=c_nA^n+c_{n-1}A^{n-1}+\ldots+c_1A+c_0I.
	\end{align*}
	\begin{enumerate}[label=\Roman*]
		\item La somma è ovvia.
		\item Il prodotto è garantito dalla commutatività delle potenze di matrici.\qedhere
	\end{enumerate}
\end{proof}
\subsection{Ideale di una matrice}
\begin{definition}{}[Ideale di una matrice]
	Data $A\in\K^{n,n}$, definiamo l'\textbf{ideale della matrice}\index{ideale!di una matrice} come
	\begin{equation*}
		I_A\coloneqq\left\{f\in\K\left[t\right]\mid f\left(A\right)=O\right\}
	\end{equation*}
\end{definition}
\begin{remark}{n}~{}
	\begin{itemize}
		\item $0\in I_A$.
		\item $I_A\neq\left\{0\right\}$; infatti, se consideriamo le seguenti $n^2+1$ matrici in $\K^{n,n}$:
		\begin{equation*}
			I,\ A,\ A^2,\ A^3,\ \ldots,\ A^{n^2}
		\end{equation*}
	Per il \textit{lemma di Steinitz} queste matrici sono necessariamente \textit{linearmente dipendenti}, dato che superano in numero $\dim \K^{n,n}=n^2$, cioè esistono i coefficienti $a_0,\ \ldots,\ a_{n^2}\in\K$ \textit{non} tutti nulli tali che
	\begin{equation*}
	...
	\item $I_A$ soddisfa giustamente la definizione di \textbf{ideale}\index{ideale} di $\K\left[t\right]$:
	\begin{itemize}
		\item \textit{$I_A$ è un sottogruppo di $\left(\K^{n, n},\ +\right)$}.
		\begin{equation*}
			\left(f+g\right)\left(A\right)=f\left(A\right)+g\left(A\right)=0\implies f+g\in I_A
		\end{equation*}
		\item \textit{Assorbimento.} Se $h\in\K\left[t\right]$ si ha
		\begin{equation*}
...
\begin{proposition}{}[Anello ${\K\left[t\right]}$ è ad ideali principali]
	L'anello $\K\left[t\right]$ è ad \textit{ideali principali}: se $I\subseteq \K\left[t\right]$ è un ideale, $\exists p$ tale che $I=\left(p\right)$. Il generatore $p$ è \textit{unico} a meno di moltiplicazione per scalari \textit{non} nulli, se prendiamo $p$ \textbf{monico} allora è unico.
\end{proposition}
\begin{proof}{n}
Sia $p\in I$ un polinomio \textit{non} nullo di grado \textit{minimo} tra i polinomi in $I_A$. Se $p$ è \textit{costante}, allora $I=\K\left[t\right]=\left(1\right)=\left(p\right)$. Supponiamo allora $p$ \textit{non} costante. Vogliamo mostrare che $p$ genera $I$. Prendiamo $f\in I$ e dividiamolo per $p$:
\begin{equation*}
	\underbrace{f\left(t\right)}_{\in I}=\underbrace{p\left(t\right)q\left(t\right)}_{\in I\text{per assorbimento}}+r\left(t\right)
\end{equation*}
Con $r\left(t\right)$ polinomio con $\deg r < \deg p$. Notiamo che anche $r\left(t\right)\in I$ per essere vera l'equazione di sopra; in particolare, per la minimalità del grado di $p$ non può esserci un polinomio in $I$ di grado minore di $p$, dunque $r\equiv 0$. Allora $p$ divide $f$ e dunque ogni polinomio in $I$ è generato da $p$: $I\equiv\left(p\right)$. Se $I=\left(p\right)=\left(\widetilde{p}\right)$, allora $p,\ \widetilde{p}\in I$ e dunque $p$ divide ed è diviso da $\widetilde{p}$, cioè $p=\lambda\widetilde{p}$ con $\lambda\in\K\setminus\left\{0\right\}$. Se $p$ è \textit{monico}, l'unico coefficiente $\lambda$ per cui si ha $p=\lambda\widetilde{p}$ è $1$, e dunque $p=\widetilde{p}$, cioè $p$ è unico.\qedhere
\end{proof}
\begin{definition}{}[Polinomio minimo]
	Sia $A\in\K^{n, n}$ e sia $I_A$ ideale dei polinomi che si annullano in $A$. Il \textbf{polinomio minimo}\index{polinomio!minimo} $m_A\left(t\right)$ di $A$ è il  \textit{generatore monico} di $I_A$, ossia è il polinomio monico \textit{non} nullo di grado minimo tra i polinomi in $I_A$.
\end{definition}
\begin{example}{n}
	Cerchiamo il polinomio minimo della seguente matrice.
	\begin{equation*}
		A=\left(\begin{array}{cc}
			0 & 1 \\
			1 & 0
...
\end{array}\right)=I
	\end{equation*}
Notiamo che $A^2-I=O$, dunque $p\left(t\right)=t^2-1\in I_A$. Poiché $m_A\left(t\right)\mid p\left(t\right)$, esso può essere solo $t-1$, $t+1$, $t^2-1$. Escludiamo sempre il caso $m_A\left(t\right)=1$, in quanto allora si avrebbe $I_A=\K\left[t\right]$ e ciò non è mai vero per questo anello: i polinomi di grado $0$ non si annullano in generale sulle matrici!. Se fosse $m_A\left(t\right)=t-1$, allora $m_A\left(A\right)=A-I\neq O$ e dunque $t-1\notin I_A$. In modo analogo se $m_A\left(t\right)=t+1$ si ha $A+I\neq 0$ e dunque $t-1\notin I_A$. L'unica possibilità è allora $m_A\left(t\right)=t^2-1$.
\end{example}
\begin{remark}{n}
	Se $A$ e $B$ sono simili, allora $I_A=I_B\subseteq \K\left[t\right]$ e quindi $m_A\left(t\right)=m_B\left(t\right)$.
\end{remark}
\begin{proof}{n}
	Sia $M\in\GL\left(n,\ \K\right)$ la matrice che rende $A$ simile a $B$: $B=M^{-1}AM$. Le potenze di matrici simili sono simili anch'esse:
	\begin{equation*}
		\begin{array}{l}
			B=M^{-1}AM\\
			B^2=M^{-1}A^2M\\
...
\end{align*}
ossia $M^{-1}p\left(A\right)M=\left(B\right)$. Pertanto, $p\left(A\right)=O$ se e solo se $p\left(B\right)=O$, cioè se $I_A=I_B$.\qedhere
\end{proof}
...
In particolare, il polinomio caratteristico è un polinomio \textit{monico}.
	\end{itemize}
\end{remember}
\begin{remark}{n}\label{polinomicoeffmatrisci}
Una matrice i cui elementi sono polinomi in $\K\left[t\right]$ può essere scritta in modo unico come polinomio in $t$ con coefficienti delle matrici in $\K^{n,n}$.
\end{remark}
\begin{example}{n}
	\begin{equation*}
		\left(\begin{array}{cc}
...
\end{array}\right)
	\end{equation*}
\end{example}
\begin{theorem}{}[Teorema di Cayley-Hamilton]\index{teorema!di Cayley-Hamilton}
Sia $A\in \mathbb{K}^{n,n}$. Allora $C_A\left(t\right)\in I_A$, cioè $C_A\left(A\right)=0$. In altre parole, $m_A\left(t\right)$ divide $C_A\left(t\right)$. In particolare, $\deg m_A\left(t\right)\leq n$.
\end{theorem}
\begin{proof}{n}
...
Poiché gli elementi di $tI-A$ sono polinomi in $\K\left[t\right]$ di grado minore o uguale di $1$ e gli elementi di $\mathrm{adj}\left(M\right)$ sono polinomi in $\K\left[t\right]$ di grado minore o uguale di $n-1$, $\mathrm{adj}\left(M\right)$ si scrive come polinomio di grado minore e uguale a $n-1$ con coefficienti in $\K^{n, n}$ grazie all'osservazione di pag. \pageref{polinomicoeffmatrisci}.
\begin{equation*}
	\mathrm{adj}\left(M\right)=C_{n-1}t^{n-1}+C_{n-2}t^{n-2}+\ldots+C_1t+C_0,\ C_i\in\K^{n,n}.
\end{equation*}
...
\begin{equation*}
	\begin{array}{ccc}
		...
		\left(tI-A\right)\left(C_{n-1}t^{n-1}+C_{n-2}t^{n-2}+\ldots+C_1t+C_0\right)&&\ovaled{t^nI+b_{n-1}t^{n-1}I+\ldots+b_1t I+b_0I}\\
		\shortparallel &&\\
...
\begin{equation*}
	...
	\end{cases}
\end{equation*}
Sostituiamo a cascata le equazioni dalla seconda in poi:
\begin{equation*}
	\begin{cases}
			C_{n-2}=A+b_{n-1}I\\
...
\end{equation*}
Abbiamo dunque ottenuto la tesi.\qedhere
\end{proof}
\begin{remark}{n}
	Poiché $m_A\left(t\right)$ divide $C_A\left(t\right)$, allora $C_A\left(t\right)=m_A\left(t\right)q\left(t\right)$, con $q\left(t\right)\in\K\left[t\right]$. In altre parole, le \textit{radici} del \textit{polinomio minimo} sono \textit{autovalori}.
\end{remark}
Il seguente teorema afferma un legame ancora più forte tra polinomio minimo e autovalori di una matrice.
\begin{theorem}{}[Radici del polinomio minimo sono autovalori di $A$ e viceversa]
	Sia $A\in \K^{n,n}$ e $m_A\left(t\right)$ il suo polinomio minimo. Allora, preso $\lambda\in\K$, $m_A\left(\lambda\right)=0$ se e solo se $\lambda\text{ è un autovalore di }A$.
\end{theorem}
\begin{proof}{n}~{}\\
	$\rightimplies$Segue dal teorema di Cayley-Hamilton perché se $m_A\left(\lambda\right)=0$, allora $C_A\left(\lambda\right)=0$ e quindi $\lambda$ è autovalore.\\
	$\leftimplies$Sia $\lambda$ un autovalore di $A$ con autovettore associato $\mathbf{v}$. Si ha
	\begin{align*}
		A\mathbf{v}&=\lambda \mathbf{v}\\
		A^2\mathbf{v}&=A\left(A\mathbf{v}\right)=A\left(\lambda \mathbf{v}\right)=\lambda A\mathbf{v}=\lambda^2 \mathbf{v}.
	\end{align*}
Iterando il procedimento, si arriva a $A^k\mathbf{v}=\lambda^k\mathbf{v}$. Un generico polinomio $p\left(t\right)\in\K\left[t\right]$ si può esprimere come
\begin{equation*}
	p\left(t\right)=\sum_{i=0}^{d}c_i t^i\quad c_i\in\K
\end{equation*}
allora
\begin{equation*}
	p\left(A\right)=\sum_{i=0}^{d}c_i A^ient
\end{equation*}
e dunque
\begin{align*}
	p\left(A\right)\mathbf{v}&=\left(\sum_{i=0}^{d}c_i A^i\right)\mathbf{v}=\sum_{i=0}^{d}c_i\left( A^i\mathbf{v}\right)=\sum_{i=0}^{d}c_i\left( \lambda^i\mathbf{v}\right)=\underbrace{\left(\sum_{i=0}^{d}c_i \lambda^i\right)}_{\in\K}\mathbf{v}=p\left(\lambda\right)\mathbf{v}
\end{align*}
...
\end{equation*}
e, essendo $v$ un autovettore, $v\neq 0$; dall'equazione sopra necessariamente segue $p\left(\lambda\right)=0$. In particolare, essendo $p\in I_A$ generato dal polinomio minimo $m_A$, cioè $p\left(t\right)=m_A\left(t\right)q\left(t\right)$ con $q\left(t\right)\neq 0$, segue che $m_A\left(\lambda\right)=0$.
\end{proof}
\section{Forma canonica di Jordan}
D'ora in poi, se non altresì specificato, considereremo $\K=\C$, cioè tratteremo di matrici $A\in \C^{n,n}$ e endomorfismi fra spazi vettoriali complessi.
\begin{remark}{n}\label{complessichiusi}
Poiché $\C$ è \textbf{algebricamente chiuso}, ogni polinomio $p\in\C\left[t\right]$ si fattorizza completamente come prodotto di fattori lineari:
\begin{equation*}
...
\end{equation*}
Come altra conseguenza, ogni matrice $n\times n$ ammette $n$ autovalori complessi, contati con le loro molteplicità.
\end{remark}
Sia $A\in \C^{n,n}$ una matrice associata a un endomorfismo $\funct{}[f]{V}{V}$. Se $f$ è diagonalizzabile, esiste una base in cui la matrice di $f$ è diagonale. Anche quando la matrice non è diagonalizzabile vogliamo comunque cercare una base in cui la matrice di $f$ sia \textit{particolarmente semplice}.
\begin{definition}{}[Blocco di Jordan]
	Un \textbf{blocco di Jordan}\index{blocco di Jordan} $J=J_k\left(\lambda\right)$, di autovalore $\lambda\in\C$ e dimensione $k$, è una matrice quadrata $k\times k$ con sulla diagonale solo l'autovalore e sopra ogni elemento della diagonale $1$:
	\begin{equation*}
...
\begin{remark}{pn}~{}
	...
	\end{itemize}
\end{remark}
\begin{remark}{n}\label{bloccojordanbase}
Definiamo il blocco di Jordan di dimensione $k$ con autovalore zero, necessario per calcolare l'autospazio $V_\lambda$:
		\begin{equation*}\setlength\arraycolsep{0.5mm}
			...
			\right)
		\end{equation*}
Si ha che $\rk N=k-1$, quindi $\dim V_{\lambda}=\dim \ker N=k-\rk N= 1$, cioè $J$ \textit{non} è \textit{mai} diagonalizzabile se $k>1$, dato che $1=\dim V_{\lambda}\leq m_\lambda = k$. Se la base $\basis$ dello spazio $V$ a $J$ è $\left\{\mathbf{e}_1,\ \ldots,\ \mathbf{e}_k\right\}$, notiamo che $\mathbf{e}_1$ è l'unico autovettore di $N$ e $V_\lambda=\mathcal{L}\left(\mathbf{e}_1\right)$. Si vede che $J$ agisce in modo particolare sui vettori di $\basis$:
\begin{equation*}
\begin{cases}
...
\begin{equation*}
	...
	\end{cases}
\end{equation*}
Cominciando da $\mathbf{e}_k$ e applicando $N$ ripetutamente otteniamo gli altri vettori della base.
\begin{center}
	\begin{tikzcd}
...
Si ha dunque, ad ogni potenza successiva di $N$, lo ‘‘spostamento'' della diagonale di $1$ verso destra. In particolare,
	...
	\right)
\end{equation*}
3 in questo caso con i vettori della base si ha la relazione 
\begin{equation*}
	\begin{cases}
	...
	\end{cases}
\end{equation*}
Studiando l'immagine dell'applicazione associata ad $N$, poiché la base dell'immagine sono i vettori colonna linearmente indipendenti, si ha $\img N^{k-1}=\mathcal{L}\left(e_1\right)$. Come già affermato dunque, è $\mathbf{e}_k$ a determinare l'\textit{intera} base di $V$ tramite la moltiplicazione per $N$. Notiamo inoltre che $N^k=O$, cioè $N$ è una matrice \textbf{nilpotente}\index{matrice!nilpotente} di ordine $k$.
\end{remark}
\begin{definition}{}[Forma di Jordan]
...
\begin{remark}{n}
	Una matrice \textit{diagonale} è in forma di Jordan, con \textit{solo} blocchi di ordine $1$ (cioè senza alcun $1$ sopra la diagonale).
\end{remark}
\begin{remark}{n}\label{molteplicitàalgebrichedijordan}
	...
	\end{equation*}
\end{remark}
\begin{theorem}{}[Esistenza e unicità della forma di Jordan]
	Sia $V$ uno spazio vettoriale complesso di $\dim n$ e $f$ un endomorfismo di $V$. Allora \textit{esiste} una base di $V$ in cui la matrice di $f$ è in forma di Jordan. Inoltre, la forma di Jordan è \textit{unica} a meno dell'ordine dei blocchi. \textit{In termini matriciali}, ogni $A\in\C^{n,n}$ è simile ad una matrice in forma di Jordan, unica a meno dell'ordine dei blocchi,
	\begin{equation*}
		J=P^{-1}AP
	\end{equation*}
	dove $P$ è la matrice del cambiamento di base che presenta, nelle colonne, la base che mette $A$ in forma di Jordan.
\end{theorem}
...
\begin{definition}{}[Spazio vettoriale invariante]
Uno spazio vettoriale $V$ si dice \textbf{invariante}\index{spazio!invariante} per un endomorfismo $f$ se $f\left(V\right)\subseteq V$. Se $A$ è la matrice associata all'endomorfismo rispetto ad una base fissata, si scrive anche $AV\subseteq V$.
\end{definition}
\begin{remark}{n}\label{observejordan}
...
\end{equation*}
\begin{itemize}
	\item $B$ è quadrata, di ordine $\dim U$ ed è associata a $\funct{}[f_{\mid U}]{U}{U}$ rispetto a $\basis_U$.
	\item $C$ è quadrata, di ordine $\dim W$ ed è associata a $\funct{}[f_{\mid W}]{W}{W}$ rispetto a $\basis_W$.
\end{itemize}
\end{remark}
\begin{definition}{}[Autospazio generalizzato]
Data una funzione $\funct{}[f]{V}{V}$ e $A$ una matrice associata ad $f$; sia $\lambda$ un autovalore di $f$,\footnote{Ne esiste almeno uno perché operiamo in $\C$.} $V_{\lambda}=\ker \left(f-\lambda Id\right)=\ker \left(A-\lambda I\right)$ l'autospazio di $\lambda$ e $m_{\lambda}$ la molteplicità algebrica di $\lambda$. Allora l'\textbf{autospazio generalizzato}\index{autospazio!generalizzato} di $\lambda$ è
\begin{equation*}
	\widetilde{V}=\ker\left(f-\lambda Id\right)^{m_{\lambda}}=\ker\left(A-\lambda I\right)^{m_{\lambda}}.
\end{equation*}
\end{definition}
\begin{property}{n}[Proprietà degli autospazi generalizzati]~{}
	\begin{enumerate}
		...
		\item $\funct{}[f_{\mid\widetilde V_{\lambda}}]{\widetilde{V}_{\lambda}}{\widetilde{V}_{\lambda}}$ ha polinomio caratteristico $\left(t-\lambda\right)^{m_{\lambda}}$.
	...
	\end{enumerate}
\end{property}
\begin{proof}{n}\label{lemmamichelegiordano}
...
\begin{enumerate}[label=\Roman*]
	...
	\end{equation*}
Al crescere di $h$
\begin{equation*}\label{catenadeinuclei}
\left\{0\right\}\subseteq \ker \left(A-\lambda I\right)\subseteq\ker \left(A-\lambda I\right)^2\subseteq\ldots,
\end{equation*}
...
\item In modo analogo, se $\mathbf{w}\in\img \left(A-\lambda I\right)^h$, per definizione esiste $\mathbf{v}\in V$ tale che
	\begin{equation*}
	\begin{array}{l}
		w=\left(A-\lambda I\right)^h\mathbf{v}=\left(A-\lambda I\right)^{h-1}\left(\left(A-\lambda I\right)\mathbf{v}\right)\\
		\implies \mathbf{w}\in\img\left(A-\lambda I\right)^{h-1}\\
		\implies \img\left(A-\lambda I\right)^{h-1}\supseteq\img\left(A-\lambda I\right)^h
	\end{array}
\end{equation*}
Al crescere di $h$
\begin{equation*}
	V\supseteq \img\left(A-\lambda I\right)\supseteq\img \left(A-\lambda I\right)^2\supseteq\ldots,
\end{equation*}
cioè l'immagine della potenza $h$ contiene tutte quelle successive. Possiamo mostrare come tutti gli spazi finora visti, ossia nuclei e immagini delle potenze $\left(A-\lambda I\right)^h$, sono invarianti:
\begin{itemize}
\item Se $\mathbf{v}\in\ker\left(A-\lambda I\right)^h$:
	...
	\end{array}
	\end{equation*}
Abbiamo dunque appena dimostrato l'invarianza dello spazio $\widetilde{V}_{\lambda}$.
\item Se $\mathbf{w}\in\img\left(A-\lambda I\right)^h$ esiste $\mathbf{v}$ tale che:
	\begin{equation*}
	\begin{array}{l}
		...
		\implies A\mathbf{w}\in \img\left(A-\lambda I\right)^h\\
		\implies A\left(\img\left(A-\lambda I\right)^h\right)\subseteq \img\left(A-\lambda I\right)^h
	\end{array}
\end{equation*}
\end{itemize}
...
\begin{gather*}
	...
	\img\left(A-\lambda I\right)^h\supseteq\img\left(A-\lambda I\right)^{h+1}
\end{gather*}
Allora, se applichiamo il \textit{teorema nullità più rango} alle applicazioni $\left(A-\lambda I\right)^h$ e $\left(A-\lambda I\right)^{h+1}$ in $V$,
\begin{equation*}
		\begin{array}{c}
			\dim \ker \left(A-\lambda I\right)^h + \dim \img\left(A-\lambda I\right)^{h}\\
			\shortparallel\\
			n=\dim V\\
			\shortparallel\\
			\dim \ker \left(A-\lambda I\right)^{h+1} + \dim \img\left(A-\lambda I\right)^{h+1}
	\end{array}
\end{equation*}
Ne consegue che
\begin{equation*}
	\ker \left(A-\lambda I\right)^h=\ker \left(A-\lambda I\right)^{h+1}\iff \img\left(A-\lambda I\right)^{h}=\img\left(A-\lambda I\right)^{h+1}.
\end{equation*}
Siccome $V$ ha dimensione finita, la successione crescente \eqref{catenadeinuclei} dei nuclei delle potenze ad un certo punto deve \textit{stabilizzarsi}, cioè deve esserci un'uguaglianza per tutti gli elementi successivi\footnote{Infatti, ogni inclusione potrebbe essere stretta e dunque la dimensione di questi sottospazi può aumentare; tuttavia, essendo $V$ finito questi sottospazio non possono avere dimensione maggiore di $n$.}. Denotiamo con $p$ il più piccolo intero tale che:
\begin{equation*}
	\ker \left(A-\lambda I\right)^p=\ker \left(A-\lambda I\right)^{p+1}
\end{equation*}
Mostriamo che per ogni $h\geq p$ valgano le seguenti relazioni:
\begin{gather*}
	\ker \left(A-\lambda I\right)^h= \ker \left(A-\lambda I\right)^p\\
	\img\left(A-\lambda I\right)^h=\img\left(A-\lambda I\right)^p
\end{gather*}
È sufficiente mostrarlo per i nuclei, dato che vale anche per le immagini per nullità più rango. Sia $\mathbf{v}\in\ker \left(A-\lambda I\right)^h\supseteq \ker \left(A-\lambda I\right)^p$ con $h\geq p+2$.\footnote{Poiché $p$ è tale per cui $\ker \left(A-\lambda I\right)^p=\ker \left(A-\lambda I\right)^{p+1}$, il caso $h=p+1$ è banalmente vero.} Allora:
\begin{equation*}
\begin{array}{l}
...
\end{itemize}
Mostriamo ora che i termini $p$-esimi delle due successioni sono in somma diretta, in particolare dobbiamo solo dimostrare che
\begin{equation*}
	\ker\left(A-\lambda I\right)^p\cap\img\left(A-\lambda I\right)^p=\left\{0\right\}.
\end{equation*}
Infatti, preso $\mathbf{u}\in\ker\left(A-\lambda I\right)^p\cap\img\left(A-\lambda I\right)^p$, $\exists\mathbf{v}\in V\ \colon \mathbf{u}=\left(A-\lambda I\right)^p\mathbf{v}$. Ma
\begin{equation*}
\begin{array}{l}
...
\end{array}
\end{equation*}
Per nullità più rango si ha $\dim \ker \left(A-\lambda I\right)^p + \dim \img\left(A-\lambda I\right)^p)=\dim V$; segue che
\begin{equation*}
V=\ker\left(A-\lambda I\right)^p\oplus\img \left(A-\lambda I\right)^p.
\end{equation*}
In particolare sappiamo che, per l'osservazione a pag. \pageref{observejordan}, rispetto ad una base di $V$ opportuna la matrice associata $A$ è \textit{a blocchi}, di cui i due non nulli sono uno \textit{codificato} dalla restrizione dell'endomorfismo a $\ker\left(A-\lambda I\right)^p$, mentre l'altro dalla restrizione a $\img\left(A-\lambda I\right)^p$. Consideriamo allora queste due restrizioni ai sottospazi:
\begin{gather*}
\funct{}[\phi]{\ker\left(A-\lambda I\right)^p}{\ker\left(A-\lambda I\right)^p}\\
\funct{}[\psi]{\img\left(A-\lambda I\right)^p}{\img\left(A-\lambda I\right)^p}
\end{gather*}
Facciamo le seguenti considerazioni.
\begin{itemize}
\item \textbf{\underline{$\lambda$ è l'unico autovalore di $\phi$.}} Definiamo la matrice $B$ associata a $\phi$. Sappiamo che $\left(A-\lambda I\right)^p$ annulla tutti i vettori di $\ker\left(A-\lambda I\right)^p$. Dunque, la \textit{restrizione} di $A-\lambda I$ su di esso, cioé $B-\lambda I$ (associata all'applicazione $\phi-\lambda Id$), è \textit{endomorfismo nilpotente} di ordine $p$. In altre parole, l'applicazione $\left(\phi-\lambda Id\right)^p$ si \textit{annulla} se valutata su un vettore (non nullo) $\mathbf{v}$ appartenente al \textit{dominio} $\ker\left(A-\lambda I\right)^p$. Ciò equivale a dire che:
\begin{equation*}
\left(B-\lambda I\right)^p\mathbf{v}=\mathbf{0}
\end{equation*}
Ma ciò significa che $\left(B-\lambda I\right)^p=\mathbf{0}$. Preso allora il polinomio $p\left(t\right)=\left(t-\lambda\right)^p$ appartiene all'ideale di $B$ (cioè all'ideale di $\phi$), in particolare $\lambda$ è autovalore di $\phi$ (perché $p\left(\lambda\right)=0\implies m_B\left(\lambda\right)=0$). Conseguentemente, se supponiamo di avere $\mu$ come altro autovalore di $\phi$, si ha che $m_B\left(\mu\right)=0\implies p\left(\mu\right)=0\implies \left(\mu-\lambda\right)^p\implies \mu=\lambda$. Si ha dunque l'unicità.
\item \textbf{\underline{$\lambda$ \textit{non} è autovalore di $\psi$.}} Infatti, sia $\mathbf{v}\in \img\left(A-\lambda I\right)$ per cui $\lambda$ è il suo autovalore. Allora
\begin{equation*}
		\begin{array}{l}
	...
	\implies \mathbf{v}\in\ker\left(A-\lambda I\right)^p\cap\img\left(A-\lambda I\right)^p=\left\{0\right\}
	\end{array}
\end{equation*}
Sapendo dunque che $\ker\left(A-\lambda I\right)^p\cap\img\left(A-\lambda I\right)^p=\left\{0\right\}$, si ha $\mathbf{v}=\mathbf{0}$, pertanto $\lambda$ \textit{non} può essere autovalore di $\psi$.
\end{itemize}
Riprendendo l'osservazione a pag. \pageref{observejordan}, scelte delle opportune basi, definiamo $\mathbf{B}$ la matrice associata a $\phi$ e $\mathbf{C}$ la matrice associata a $\psi$ in modo da avere la matrice $A$ associata a $f$ a blocchi.
\begin{equation*}
	A = \left(
	\begin{array}{c|c}
		\mathbf{B} & \mathbf{0}\\
...
\begin{itemize}
	...
	\end{equation*}
	\item $C_C\left(t\right)$, in quanto $\psi$ non ha l'autovalore $\lambda$, non è divisibile per $t-\lambda$.
\end{itemize}
...
\item Notiamo che l'endomorfismo $\phi$ definito nella dimostrazione precedente altro non è che $\funct{}[f_{\mid\widetilde V_{\lambda}}]{\widetilde{V}_{\lambda}}{\widetilde{V}_{\lambda}}$, e abbiamo visto come il suo polinomio caratteristico debba essere $\left(t-\lambda\right)^{m_{\lambda}}$.
\item Non dimostreremo quest'ultimo punto.\qedhere
\end{enumerate}
\end{proof}
...
\end{equation*}
Ora, per trovare una base che mette la matrice $A$ associata ad $f$ in forma di Jordan, basta farlo in \textit{ogni autospazio generalizzato} $\widetilde{V}_{\lambda_i}$, in cui l'unico autovalore è $\lambda_i$ per le osservazioni precedenti. In sostanza, quello che vogliamo fare è compiere una \textit{‘‘separazione degli autovalori''}. Per calcolare l'autospazio generalizzato dovremmo calcolare $\widetilde{V}_{\lambda}=\left(A-\lambda I\right)^{m_{\lambda}}$, ma basterà calcolare invece $\widetilde{V}_{\lambda}=\left(A-\lambda I\right)^{p}$. Nella sezione seguente dimostreremo l'esistenza della base di $\widetilde{V}_{\lambda}$ che dà la forma di Jordan.
...
\begin{lemma}{}[Dimensione intersezione immagine-nucleo di due funzioni]\label{lemmadimjordan}
Siano $\funct{}[f]{U}{V}$ e $\funct{}[g]{V}{W}$ due applicazioni lineari. Si ha:
\begin{equation*}
\dim\left(\img f\cap \ker g\right)=\dim\img f-\dim\img \left(g\circ f\right)=\dim \ker\left(g\circ f\right)-\dim\ker f
\end{equation*}
\end{lemma}
\begin{proof}{n}
\[\begin{tikzcd}
...
\end{tikzcd}\]
Sia $h\coloneqq \funct{}[g_{\mid \mathrm{Im} f}]{\mathrm{Im} f}{W}$. Si ha
\begin{equation*}
	\dim \ker h=\dim \img f-\dim \img h,
\end{equation*}
ma $\ker h=\img f\cap\ker g$ e $\img h=g\left(\img f\right)=\img\left(g\circ f\right)$, dunque
\begin{equation*}
	\begin{array}{l}
	\dim \ker h=\dim \img f-\dim \img h\\
	\dim \left(\img f\cap\ker g\right) =\dim \img f-\dim \img\left(g\circ f\right).
	\end{array}
\end{equation*}
Per dimostrare la seconda uguaglianza, abbiamo
\begin{equation*}
	\begin{array}{l}
		\dim \img f=\dim U-\dim \ker f\\
		\dim \img\left(g\circ f\right)=\dim U-\dim \ker\left(g\circ f\right)\\
		\implies \dim \img f-\dim \img\left(g\circ f\right)=\dim \ker\left(g\circ f\right)-\dim \ker f
	\end{array}\qedhere
\end{equation*}
\end{proof}
\begin{proof}{n}
	Ricordando la successione delle immagini \eqref{successionejordan}
	\begin{gather*}
		V\supsetneqq \img\left(A-\lambda I\right)\supsetneqq\ldots\supsetneqq\img \left(A-\lambda I\right)^p
	\end{gather*}
	intersechiamo ogni termine con $V_{\lambda}=\ker\left(A-\lambda I\right)$
	\begin{equation*}
		\ker\left(A-\lambda I\right)\cap V\supseteq \ker\left(A-\lambda I\right)\cap\img \left(A-\lambda I\right)\supseteq\ldots\supseteq\ker\left(A-\lambda I\right)\cap\img \left(A-\lambda I\right)^p.
	\end{equation*}
	e poniamo
	\begin{equation*}
		S_i\coloneqq \ker\left(A-\lambda I\right)\cap \img\left(A-\lambda I\right)^{i-1}.
	\end{equation*}
	In particolare, notiamo che:
	\begin{itemize}
		\item $S_1=\ker\left(A-\lambda I\right)\cap V=\ker\left(A-\lambda I\right)=V_{\lambda}$.
		\item $S_{p+1}=\ker\left(A-\lambda I\right)\cap \img\left(A-\lambda I\right)^{p}=\left\{\mathbf{0}\right\}$ perché $\ker\left(A-\lambda I\right)\subsetneqq \ker\left(A-\lambda I\right)^p$ e dunque $S_{p+1}\subseteq \ker\left(A-\lambda I\right)^p\cap \img\left(A-\lambda I\right)^{p}=\left\{\mathbf{0}\right\}$.
...
\end{equation*}
Costruiamo la base di $\widetilde{V}_{\lambda}$. Innanzitutto, scegliamo una base $\left\{x_1^1,\ \ldots,\ x_r^1\right\}$ del sottospazio più piccolo $S_p$. Per costruzione, $x^1_i\in\img\left(A-\lambda I\right)^{p-1}$, cioè
\begin{equation*}
...
\begin{center}
	...
	\end{tikzcd}
\end{center}
Notiamo che i vettori $\left\{x_i^1,\ \ldots,\ x_i^P\right\}$ danno origine ad un \textit{blocco di Jordan} $J_p\left(\lambda\right)$ di dimensione $p$ e relativo all'autovalore $\lambda$, poiché questi vettori soddisfano la costruzione vista nell'osservazione di pag. \pageref{bloccojordanbase}: infatti, si ha $x_i^1\in S_p\subseteq V_{\lambda}$, dunque $x_i^1$ è un autovettore di $V_\lambda$ e gli altri vettori sono ottenuti dall'applicazione ripetuta di una matrice all'ultimo vettore della base\footnote{Chiaramente ciò non implica che il blocco di Jordan in esame sia proprio $A$! $A$ ha sempre ordine $n\times n$, mentre il blocco ottenuto dalla base in questione ha ordine $p\times p$, con $p\leq n$. }. Lo stesso vale $\forall i=1,\ \ldots,\ r$. Consideriamo ora lo spazio $S_{p-1}$, che ricordiamo contiene $S_{p}$ (cioè $S_p\subseteq S_{p-1}$). Vogliamo completare $\left\{x_1^1,\ \ldots,\ x_r^1\right\}$ ad una base di $S_{p-1}$ con dei vettori $y_1^1,\ \ldots,\ y_s^1$:
\begin{equation*}
	\left\{x_1^1,\ \ldots,\ x_r^1,\ y_1^1,\ \ldots,\ y_s^1\right\}
\end{equation*}
Per costruzione, $y_j^1\in S_{p-1}\subseteq \img\left(A-\lambda I\right)^{p-2}$, dunque:
\begin{equation*}
	\forall j=1,\ \ldots,\ s\ \exists y_1^{p-1}\in V\quad y_j^1=\left(A-\lambda I\right)^{p-2}y_j^{p-1}
\end{equation*}
Per ogni $j$ otteniamo $p-1$ vettori  $\left\{y_j^1,\ \ldots,\ y_j^{p-1}\right\}$ tali che $y^s_j\in V_{\lambda}$ e $y_j^{i-1}\coloneqq\left(A-\lambda I\right)y_j^i$ $\forall i=2,\ \ldots,\ p-1$. Analogamente al caso precedente, questo gruppo di vettori dà origine ad un \textit{blocco di Jordan} di ordine $p-1$. Procediamo in questo modo: prendiamo la base ottenuta per $S_{i}$ e la completiamo ad una di $S_{i-1}\supseteq S_{i}$; poiché ogni vettore aggiunto appartiene a $\img\left(A-\lambda I\right)^{i-2}$, applicando $i-2$ volte la matrice $A-\lambda I$ al vettore $z_k^{i-1}$ (fino ad ottenere $z_k^1$) otteniamo un'insieme di vettori che generano un blocco di Jordan di dimensioni $i$ e di autovalore $\lambda$. Chiaramente, poiché potrebbe anche accadere che $S_{i-1}= S_{i}$, si prosegue senza aggiungere vettori alla base e si passa al sottospazio successivo. Arriviamo con queste iterazioni fino a $S_2=V_\lambda\cap \img\left(A-\lambda I\right)$: completiamo la base da $S_3$ ad una di $S_2$ aggiungendo i vettori $\left\{a_1^1,\ \ldots,\ a_t^1\right\}$. Sappiamo che $\exists a_t^2\ \colon a_t^1=\left(A-\lambda I\right)^{p-2}a_t^2$, dunque abbiamo i due vettori che formano il blocco di Jordan di dimensione $2$. Infine, completiamo ad una base di $S_1$ aggiungendo i vettori $\left\{b_1^1,\ \ldots,\ b_u^1\right\}$. In questo caso, non abbiamo bisogno di calcolare altri vettori $b_u^i\ \forall u$ (al variare di $i$) come prima, in quanto i vettori, per definizione di $S_1$, appartengono anche a $\ker\left(A-\lambda I\right)$. Allora, $\forall u\ b_u^1$ generano blocchi di Jordan di dimensione $1$. Al variare di $i,\ j,\ k,\ \ldots,\ t,\ u$ abbiamo costruito un insieme di vettori tutti appartenenti a $\widetilde{V}_{\lambda}=\ker\left(A-\lambda I\right)^p$: nello schema precedente essi sono tutti i vettori appartenenti a tutte le colonne, da quella di $x_i$ a quella di $b_u$.\\
\textbf{\underline{Vogliamo contare quanti sono questi vettori.}} Innanzitutto, dobbiamo considerare che lo schema, per compattezza, rappresenta \textit{solo una colonna} per ciascun $x_i,\ y_j,\ \ldots$, ma in realtà c'è una colonna analoga alla prima \textit{per ogni vettore} della base di $S_p$, una colonna analoga alla seconda per ogni vettore della base di $S_{p-1}$ e così via. In pratica, abbiamo $\dim S_p=r$ colonne con $x_i$, $\dim S_{p-1}-\dim S_{p}=s$ colonne con $y_j$ e così via. Contiamo adesso gli elementi per \textit{righe}. L'\textit{ultima riga}, quella di $x_i^1,\ y_j^1,\ z_k^1,\ \ldots,\ a_t^1,\ b_u^1$ al variare di $i,\ j,\ k,\ \ldots,\ t,\ u$, sono per costruzione i vettori di una base di $S_1$, e quindi il loro numero sono $\dim S_1$. Sulla \textit{penultima riga} non abbiamo i vettori $b_u$ e i vettori $x_i^2,\ y_j^2,\ z_k^2,\ \ldots,\ a_t^2$ presenti sono in numero uguale ai vettori $x_i^1,\ y_j^1,\ z_k^1,\ \ldots,\ a_t^1$ al variare di $i,\ j,\ k,\ \ldots,\ t$, base di $S_2$ e quindi ne abbiamo $\dim S_2$. Proseguendo così, il numero di vettori della $i$-esima riga è pari alla dimensione dello spazio $S_i$; in totale l'insieme è formato da $N$ vettori, con
\begin{equation*}
	N=\sum_{i=1}^{p}\dim S_i.
\end{equation*}
Usando il lemma \ref{lemmadimjordan} (pag. \pageref{lemmadimjordan}), otteniamo che
\begin{align*}
	\dim S_i&=\dim \left(\ker\left(A-\lambda I\right)\cap \img\left(A-\lambda I\right)^{i-1}\right)=\\
...
\end{equation*}
Applicando $\left(A-\lambda I\right)^{p-2}$, nella combinazione lineare rimangono solo $x_i^{p-1}$ e $y_j^{p-1}$ al variare di $i$ e $j$ con i loro coefficienti. Complessivamente, i vettori formano la base già vista di $S_{p-1}$, dunque i coefficienti risultano nulli: $\beta_i=0,\ \gamma_j=0\ \forall i,\ j$. Allo stesso modo, applicando $\left(A-\lambda I\right)^{p-3},\ \ldots$ si vede che tutti i coefficienti della combinazione lineare sono nulli, ossia i vettori dell'insieme sono \textbf{linearmente indipendenti}.\qedhere
\end{proof}
\subsection{Unicità della forma di Jordan}
\begin{proof}{n}
Per ultima cosa osserviamo come la forma di Jordan di $A$ sia unica. Sulla sua diagonale compaiono, per definizione, gli \textit{autovalori con molteplicità}: questo dipende esclusivamente dalle radici del polinomio caratteristico e dunque da $A$ stessa. Per un dato autovalore $\lambda$, abbiamo ottenuto dei blocchi di Jordan corrispondenti agli spazi $S_k$ di dimensione $k$ e di numero pari ai vettori aggiunti per completare la base dello spazio $S_{k+1}$ passo per passo, ossia $\dim S_{k-1}-\dim S_k$, dato che ogni vettore aggiunto $x_i^1$ genera la successione $x_i^1,\ \ldots,\ x_i^k$. Poiché il \textit{numero dei blocchi} dipende esclusivamente da $A-\lambda I$, dunque da $A$ stessa, e \textit{non} dal procedimento, la forma di Jordan di $A$ è unica.\qedhere
\end{proof}
Il corollario seguente è immediato.
\begin{corollary}{q}[Similitudine di matrici e forma di Jordan]
Due matrici in forma di Jordan sono simili se e solo se hanno gli stessi blocchi, a meno dell'ordine.\qedhere
\end{corollary}
...
\begin{remark}{n}
	Se $p\left(t\right),\ q\left(t\right)\in\K\left[t\right]$, allora
	\begin{equation*}
		p\left(A\right)q\left(A\right)=q\left(A\right)p\left(A\right).
	\end{equation*}
\end{remark}
\begin{proof}{n}
	Possiamo supporre che $A$ sia già in forma di Jordan. Consideriamo $A-\lambda_1I$, rappresentata in figura: ha, nella parte rossa, dei blocchi di Jordan relativi all'autovalore zero. Poiché essa è una sottomatrice nilpotente di ordine $p_1$, ne consegue che $\left(A-\lambda_1I\right)^{p_1}$ ha la matrice nulla nella parte rossa. In generale, $\left(A-\lambda_iI\right)^{p_i}$ è nullo nel blocco $m_i\times m_i$ corrispondente a $\lambda_i$.
	 \begin{equation*}
	 	\tikz[baseline]{
	 		\node[matrix of math nodes,matrix anchor=west,left delimiter=(,right delimiter=),ampersand replacement=\&] (M) {
	 			~\& 						\& \& \& \& \& 	\\
...
\end{equation*}
Perciò il polinomio minimo $m_A$ divide $f$. Consideriamo ora $\left(A-\lambda_1I\right)^h$ con $h<p_1$: come abbiamo visto nello studio delle proprietà dei blocchi di Jordan, esso ha nel primo blocco una colonna uguale a $\mathbf{e}_1=\left(1,\ 0,\ \ldots,\ 0\right)^\mathsf{T}$, diciamo ad esempio la colonna $s\in\left\{1,\ \ldots,\ m_1\right\}$. Posto $d_i\geq 1$, $\left(A-\lambda_iI\right)^{d_i}$ nel posto $\left(1,\ 1\right)$ ha $\left(\lambda_1-\lambda_i\right)^{d_i}\neq 0$ se $i=2,\ \ldots, \ r$. Infatti, $A$ (presa in forma di Jordan) è triangolare superiore e ha $\lambda_1$ al posto $\left(1,\ 1\right)$; allo stesso modo $A-\lambda_i I$ è triangolare superiore e ha $\lambda_1-\lambda_i$ al posto $\left(1,\ 1\right)$. Ne consegue che 
\begin{equation*}
	\prod_{i=2}^{r}\left(A-\lambda_iI\right)^{d_i}
\end{equation*}
ha un numero $\neq 0$ nel posto $\left(1,\ 1\right)$. Allora, utilizzando l'osservazione ad inizio sezione che garantisce la commutatività del prodotto si ha
\begin{equation*}
...
\end{equation*}
Segue che qualunque blocco di Jordan di ordine non massimo fa sì che il polinomio scritto sopra non appartenga all'ideale di $A$, e dunque il più piccolo polinomio che è diviso da $m_A\left(t\right)$ (al quale dunque deve coincidere necessariamente) è $f\left(t\right)$ visto sopra.\qedhere
\end{proof}
\begin{corollary}{}[Molteplicità delle radici del polinomio minimo e diagonalizzabilità]
Sia $A\in\C^{n,n}$. Allora $A$ è diagonalizzabile se e solo se il suo polinomio minimo ha tutte radici di molteplicità $1$.
\end{corollary}
\begin{proof}{n}~{}\\
$\rightimplies$Se $A$ è diagonalizzabile, tutti i blocchi sono di dimensione $1$ e questa, per la stessa proposizione di prima, corrisponde alla molteplicità delle radici del polinomio caratteristico.\\
$\leftimplies$Per la proposizione precedente, la molteplicità delle radici del polinomio minimo corrisponde alla dimensione del più grande blocco di Jordan di $A$ relativo a $\lambda_i$. Segue chiaramente che se $m_{\lambda_i}=1\ \forall i$ l'ordine di tutti i blocchi è $1$, dunque $A$ è diagonalizzabile.\qedhere
\end{proof}
\begin{remark}{n}
...
\begin{tipsandtricks}{n}[Alcune nozioni utili per il calcolo della base e della forma di Jordan]~{}
	\begin{enumerate}
		...
		\end{equation*}
		\item La base dell'autospazio $V_{\lambda}=S_1$ sono tutti i vettori aggiunti a partire dalla base di $S_p$, compresi i vettori di quest'ultima base; poiché per ognuno di questi vettori abbiamo, per costruzione, un blocco di Jordan relativo a $\lambda$, il numero di questi vettori corrisponde al \textit{numero totale di blocchi di Jordan, cioè la molteplicità geometrica di} $\lambda$:
		\begin{equation*}
		...
		\end{equation*}
		\item \textit{Se conosciamo già le dimensioni dei blocchi di Jordan} di $\lambda$,
		\begin{equation*}
			a_1\leq\ldots\leq a_r=p
		\end{equation*}
		\textit{è sufficiente calcolare i sottospazi:}
		\begin{equation*}
		...
		\begin{equation*}
			\begin{array}{l}
				\forall \mathbf{v}\in\img\left(A-\lambda I\right)^{p-1}\ \exists\mathbf{u}\in V\ \colon \left(A-\lambda I\right)^{p-1}\mathbf{u}=\mathbf{v}\\
				...
				\implies \img\left(A-\lambda I\right)^{p-1}\subseteq \ker\left(A-\lambda I\right)\\
				\implies S_p=\img\left(A-\lambda I\right)^{p-1}\cap \ker\left(A-\lambda I\right)=\img\left(A-\lambda I\right)^{p-1}.
			\end{array}
		\end{equation*}	
		\textit{Pertanto, nel caso} $S_p$ \textit{non c'è bisogno di intersecare con} $V_{\lambda}$! Questo tuttavia \textit{non} si applica agli altri $S_i$, dato che \textit{non} vale la relazione $\img\left(A-\lambda I\right)^{i}\subseteq \ker\left(A-\lambda I\right)$.
		\item Se so che per $\lambda$ tutti i blocchi di Jordan hanno la stessa dimensione $p$, \textit{possiamo calcolare direttamente} $S_p=\img\left(A-\lambda I\right)^{p-1}\cap V_{\lambda}$. %(dato che $S_p=S_{p-1}=\ldots=S_1$ e dunque non ci sono blocchi di altre dimensioni)?
	\end{enumerate}
\end{tipsandtricks}
\begin{exercise}{n}
Sia data la matrice:
...
\begin{equation*}
V_2=S_1\supseteq S_2=\img\left(A-2 I\right)
\end{equation*}
Avendo un unico autovalore, nel caso $S_2$ non abbiamo bisogno di calcolare l'intersezione con l'autospazio. Dunque, cerchiamo una base di $S_2=\img\left(A-2 I\right)$. Sappiamo già che la sua base è di un solo vettore, dato che $\rk\left(A-2 I\right)=1=\dim\img\left(A-2 I\right)$. Essendo l'immagine, possiamo prendere un vettore colonna della matrice $A-2 I$, che definiremo $x_1^1$; ad esempio, prendiamo la prima colonna:
\begin{equation*}
...
\end{equation*}
\end{solution}
\begin{exercise}{}[Esercizio 4, scritto Luglio 2018]
...
\begin{equation*}
	\lambda_{1,\ 2}=\frac{1\pm \sqrt{1+4}}{2}=\frac{1\pm\sqrt{5}}{2}\in\R
\end{equation*}
...
La \textbf{funzione esponenziale} $e^x$, con $x\in \R$ si può \textit{caratterizzare} in diversi modi; sia con il concetto di limite...
\begin{equation*}
	e^x=\lim_{n \to +\infty}\left(1+\frac{x}{n}\right)^n
\end{equation*}
... sia come il valore della serie di potenze:
\begin{equation*}
	e^x=\sum_{n=0}^{+\infty}\frac{x^n}{n!}=1+x+\frac{x^2}{2}+\frac{x^3}{3!}+\ldots
\end{equation*}
Vogliamo ora definire una funzione analoga anche in campo complesso.
\begin{definition}{}[Funzione esponenziale sui numeri complessi]
Sia $z\in\C$. Definiamo come \textbf{funzione esponenziale sui numeri complessi}\index{funzione!esponeziale sui numeri complessi} la seguente serie:
\begin{equation*}
...
\begin{proof}{n}
	...
	\begin{equation*}
		\abs{\frac{z^n}{n!}} =\frac{\lvert z \rvert^n}{n!}\in\R\implies\sum_{n=0}^{+\infty}\frac{\lvert z \rvert^n}{n!}
	\end{equation*}
Questa serie nei reali converge ad $e^{\lvert z\rvert}$: la serie pertanto converge assolutamente e dunque la funzione è ben definita; se $z\in \R$ allora l'esponenziale è in tutto e per tutto quello noto nei reali. Studiamo ora la continuità, dimostrando che \textbf{converga uniformemente}\footnote{Nelle ‘‘Note aggiuntive'', a pag. \pageref{convergenzauniforme}, si può trovare la definizione della convergenza uniforme e alcune osservazioni a riguardo.} in qualunque sottoinsieme limitato, utilizzando l'\textit{M-test di Weierstrass}. Se $S\subseteq \C$ è un sottoinsieme limitato, sicuramente esso è sottoinsieme di un disco nel piano complesso di centro l'origine e raggio $\epsilon$. Dunque, $\exists\epsilon\in\R\ \colon \lvert z\rvert<\epsilon\ \forall z\in S$. Allora varrà
\begin{equation*}
\norm{\frac{z^n}{n!}}=\frac{\lvert z\rvert^n}{n!}\leq\frac{\epsilon^n}{n!}.
\end{equation*}
Passando alle serie
\begin{equation*}
\sum_{n=0}^{+\infty}\frac{\epsilon^n}{n!}<\infty.
\end{equation*}
La funzione esponenziale converge uniformemente su $S$, e pertanto $\funct{}[e^z]{\C}{\C}$ è continua.\qedhere
\end{proof}
...
\begin{enumerate}
	...
	\item $e^z\neq 0\ \forall z\in \C$.
	\item Se $t\in\R$, si ha $e^{it}=\cos t+i\sin t$.
\end{enumerate}
\end{proposition}
\begin{proof}{n}~{}
	\begin{enumerate}[label=\Roman*]
		\item Dati $z,\ w\in\C$:
		\begin{align*}
				 e^z\cdot e^w& =\sum_{k=0}^{\infty}\frac{z^k}{k!}\cdot\sum_{m=0}^{\infty}\frac{w^m}{m!}\stackrel{\footnote{Il prodotto è lecito in quanto si ha la convergenza assoluta della serie.}}{=}\underbrace{\sum_{n=0}^{\infty}}_{n=k+m}\sum_{k=0}^{n}\frac{z^k}{k!}\frac{w^{n-k}}{\left(n-k\right)!}=\\
				& =\sum_{n=0}^{+\infty}\underbrace{\frac{1}{n!}\sum_{k=0}^{n}{n \choose k}z^kw^{n-k}}_{\text{Binomio di Newton}}=\sum_{n=0}^{+\infty}\frac{1}{n!}\left(z+w\right)^n=e^{z+w}\\
				& \implies  e^z\cdot e^w=e^{z+w}
		\end{align*}
	\item $\forall z,\ e^z\cdot e^{-z}=e^{z-z}=e^0=1$.
	\item Si ha:
	\begin{align*}
			 e^{it}=&\displaystyle=\sum_{n=0}^{+\infty}\frac{\left(it\right)^n}{n!}=\sum_{m=0}^{+\infty}\frac{\left(it\right)^{2m}}{\left(2m\right)!}+\sum_{m=0}^{+\infty}\frac{\left(it\right)^{2m+1}}{\left(2m+1\right)!}=\\
			&=\sum_{m=0}^{+\infty}\frac{\left(-1\right)^m\left(t\right)^{2m}}{\left(2m\right)!}+i\sum_{m=0}^{+\infty}\frac{\left(-1\right)^m\left(t\right)^{2m+1}}{\left(2m+1\right)!}=\cos t + i\sin t\qedhere
	\end{align*}
	\end{enumerate}
...
\begin{remark}{pn}~{}
	\begin{itemize}
		\item $e^z=e^{x+iy}=e^x\cdot e^{iy}=e^x\left(\cos y+i\sin y\right)\implies \lvert e^z\rvert = e^x = e^{\Re z}$.	L'argomento di $e^z$ è, per costruzione, $y=\Im z$.
		\item $e^{2\pi i}=1$, mentre $e^{z+2\pi i}=e^z\cdot e^{2\pi i}=e^{z}$.
		\item $e^z\neq 0$, dunque $\forall w\in \C\setminus \left\{0\right\},\ \exists z\in \C\ \colon e^z=w$, cioè $\funct{}[e^z]{\C}{\C\setminus\left\{0\right\}}$ è suriettiva. Infatti, se $w=x+iy$ si può scrivere in forma polare come
		\begin{equation*}
		...
		\begin{itemize}
			...
			\item $\lvert w\rvert=\sqrt{x^2+y^2}\in\R^{+}$, dunque per suriettività dell'esponenziale reale $\exists a\in\R$ tale per cui $e^a=\sqrt{x^2+y^2}$.
...
\begin{definition}{}[Esponenziale di una matrice quadrata complessa]
Sia $A\in\C^{n,n}$. Definiamo l'\textbf{esponenziale di una matrice quadrata complessa} come
\begin{equation*}
	e^A\coloneqq \sum_{k=0}^{+\infty}\frac{A^k}{k!}=\lim_{n \to +\infty}\sum_{k=0}^{n}\underbrace{\frac{A^k}{k!}}_{\text{matrice }n\times n}\qquad e^A\in\C^{n,n}.
\end{equation*}
\end{definition}
...
\begin{definition}{}[Norma infinito di una matrice]
La \textbf{norma infinito di una matrice}\index{norma!infinito di una matrice} $A\in \C^{n,n}$ è
\begin{equation*}
\norm{A}_{\infty}=\max_{i,\ j=1,\ \ldots,\ n}\lvert a_{ij}\rvert
\end{equation*}
\end{definition}
\begin{property}{}[Proprietà della norma infinito di una matrice]
	Date le matrici $n\times n$ $A$ e $B$:
	\begin{enumerate}
		\item $\norm{A+B}_{\infty}\leq \norm{A}_{\infty}+\norm{B}_{\infty}$.
		\item $\norm{A\cdot B}_{\infty}\leq n\norm{A}_{\infty}\norm{B}_{\infty}$.
	\end{enumerate}
\end{property}
\begin{proof}{n}~{}
	\begin{enumerate}[label=\Roman*]
		\item $\forall i,\ j$ $\lvert a_{ij}+b_{ij}\rvert\leq \lvert a_{ij}\rvert + \lvert b_{ij}\rvert\leq \norm{A}_{\infty}+\norm{B}_{\infty}$.
		Per l'arbitrarietà di $i$ e $j$, vale la tesi.
		\item Sia $C=AB$. Allora:
		\begin{align*}
				\displaystyle c_{ij}&=\sum_{k=1}^{n}a_{ik}b_{kj}\\
				\displaystyle\implies \lvert c_{ij}\rvert &\leq \sum_{k=1}^{n}\lvert a_{ik}\rvert \lvert b_{kj}\rvert\leq n\norm{A}_{\infty}\norm{B}_{\infty}\quad \forall i,\ j\\
				\implies \norm{C}_{\infty}&\leq n\norm{A}_{\infty}\norm{B}_{\infty}\qedhere
		\end{align*}
	\end{enumerate}
\end{proof}
\begin{proof}{n}
...
\begin{equation*}
	\begin{array}{l}
			\norm{A^2}_{\infty}\leq n\norm{A}^2_{\infty}\\
			\\[-3mm]
			\norm{A^3}_{\infty}\leq n\norm{A^2\cdot A}_{\infty}\leq n\norm{A}^2_{\infty}\norm{A}_{\infty}\leq n^2\norm{A}^3_{\infty}\\
	\end{array}
\end{equation*}
Per induzione in questo modo otteniamo $\norm{A^k}_{\infty}\leq n^{k-1}\norm{A}^k_{\infty}$, da cui
\begin{align*}
		\sum_{k=0}^{N}\norm{\frac{A^k}{k!}}_{\infty}&=\sum_{k=0}^{N}\frac{\norm{ A^k}_{\infty}}{k!}\leq\sum_{k=0}^{N}\frac{n^{k-1}\norm{A}^k_{\infty}}{k!}=\\
		&=\frac{1}{n}\sum_{k=0}^{N}\frac{\left(n\norm{A}_{\infty}\right)^k}{k!}\stackrel{\longrightarrow}{N\to \infty} \frac{1}{n}\sum_{k=0}^{\infty}\frac{\left(n\norm{A}_{\infty}\right)^k}{k!}=\frac{1}{n}e^{n\norm{A}_{\infty}}
\end{align*}
Allora
\begin{equation*}
	\sum_{k=0}^{\infty}\frac{A^k}{k!}
\end{equation*}
converge assolutamente, pertanto $e^A$ è ben definito.\qedhere
\end{proof}
\begin{warning}{n}
...
\end{warning}
\begin{example}{n}\label{esponenzialechenoncommutap1}
	Siano
	\begin{equation*}
		A= \left(
		\begin{array}{cc}
...
\end{example}
\begin{lemma}{}[Esponenziale di matrici che commutano]
Se $A,\ B\in\C^{n,n}$ \textit{commutano}, cioè $AB=BA$, allora:
	\begin{equation*}
		e^{A+B}=e^A\cdot e^B
	\end{equation*}
\end{lemma}
\begin{proof}{n}
	...
	\begin{equation*}
		\left(A+B\right)^k=\sum_{i=0}^{k}{k \choose i}A^i\cdot B^{k-1}\qedhere
	\end{equation*}
\end{proof}
\begin{remark}{n}
...
\begin{align*}
		A^k&=P^{-1}B^kP\\
		\implies e^A&\displaystyle=\sum_{k=0}^{N}\frac{A^k}{k!}=\sum_{k=0}^{N}\frac{P^{-1}B^kP}{k!}=P^{-1}\sum_{k=0}^{N}\frac{B^k}{k!}P=P^{-1}e^BP
\end{align*}
\end{proof}
\begin{theorem}{}[Determinante di un esponenziale matriciale]
	Si ha
	\begin{equation*}
		\det\left(e^A\right)=e^{\trc\left(A\right)}.
	\end{equation*}
In particolare, $e^A$ è sempre una matrice invertibile.
\end{theorem}
\begin{proof}{n}
	Una qualunque matrice $A$ complessa è simile alla sua forma di Jordan $J$. La traccia di matrici simili, per commutatività interna della traccia\footnote{Per ogni matrice $A$ di dimensioni $n\times m$ e $B$ di dimensioni $m\times n$ si ha $\trc\left(AB\right)=\trc\left(BA\right)$.}, è uguale:
	\begin{equation*}
		\trc\left(A\right)=\trc\left(J\right)=\lambda_1+\ldots+\lambda_n
	\end{equation*}
...
\begin{equation*}
	...
	\end{array}\right)
\end{equation*}
Il determinante di una matrice triangolare è il prodotto sulle colonne, dunque vale $\det\left(e^J\right)=e^{\lambda_1}\ldots e^{\lambda_n}$ come cercato. In particolare, questo prodotto, in quanto \textit{prodotto di esponenziali}, \textit{non è mai nullo} e dunque il determinante è \textit{diverso da zero}.\qedhere
\end{proof}
...
\end{equation*}
	...
	\end{equation*}
\end{itemize}
In generale, tuttavia, come possiamo calcolare l'esponenziale di una generica matrice $A$? A questo proposito ci viene in aiuto la tanto faticata forma di Jordan. Il seguente processo costruttivo ci permette di calcolare, in modo relativamente facile, l'esponenziale $e^A$.
\begin{enumerate}
...
\end{itemize}
\end{enumerate}
\begin{example}{n}\label{esponenzialechenoncommutap2}
...
1 & 1 \\
0 & 2 \\
\end{array}\right)$. $C$ ha autovalori $1$ e $2$ ed è diagonalizzabile con $D=\left(\begin{array}{ccc}
	1 & 0 \\
	0 & 2 \\
...
\begin{exercise}{}[Esercizio 4, scritto Febbraio 2018]
Sia $A =
	e^{\lambda I}=\left(\begin{array}{ccc}
		-3 & 4 \\
		-1 & 1 \\
...
\begin{equation*}
	...
	\end{array}\right)
\end{equation*}
Cerchiamo ora una matrice $P$, e dunque una base $\basis$, che mette $A$ in forma di Jordan. Poiché abbiamo un unico autovalore, $\left(A-\lambda I\right)^2=O$ e $\img\left(A-\lambda I\right)\subseteq \ker \left(A-\lambda I\right)$. Studiamo $S_2=\ker\left(A+I\right)\cap \img\left(A+I\right)^{p-1}=\img\left(A+I\right)$; esso ha $\dim S_2=1$ e per trovarne una base basta prendere una colonna di $A+I$:
\begin{equation*}
v_2 = \left(2,\ 1\right)=
\end{equation*}
...
\end{array}\right)
\end{equation*}
L'inversa, noto il determinante $\det P= 1$, è
\begin{equation*}
...
\begin{equation*}
	...
	\end{array}\right)
\end{equation*}
Si verifica facilmente che usando questa matrice $P$ si arriva comunque allo stesso esponenziale visto prima. In questo problema si può anche evitare il calcolo della forma di Jordan. Infatti, notando che la matrice $B=A+I$ è nilpotente di ordine $2$, ovvero $B^2=O$, e commuta con $-I$. Allora possiamo calcolare $e^A$ in questo modo:
\begin{equation*}
...
Abbiamo studiamo le forme di Jordan in $\C^{n,n}$, dato che abbiamo la sicurezza dell'esistenza di tutti gli autovalori e dunque anche della forma di Jordan. E se la matrice fosse a valori reali, possiamo parlare di forma di Jordan in $\R^{n,n}$? Dato che la forma di Jordan associata ad una matrice ha sulla diagonale gli autovalori di $A$ con molteplicità e al di fuori di essa o zero o uno, possiamo fare la seguente osservazione.
\begin{remark}{n}
Sia $A\in\R^{n,n}$ e $J$ la forma di Jordan di $A$. Allora $J$ è \textit{reale} se e solo se gli autovalori di $A$ sono \textit{reali}.
\end{remark}
Supponiamo che $A$ abbia autovalori reali e $J$ sia la sua forma di Jordan. Allora esiste $P\in\GL\left(n,\ \C\right)$ tale che esse siano simili per $P$ in campo complesso: $A=PJP^{-1}$. In realtà, si può dimostrare come $A$ e $J$ siano simili come matrici reali, cioè esiste $ Q\in\GL\left(n,\ \R\right)$ tale che $A=QJQ^{-1}$
\begin{theorem}{}[Matrici reali simili nei complessi lo sono nei reali]
Siano $A,\ B\in\R^{n,n}$ tali che esiste $P\in\GL\left(n,\ \C\right)$ tale che $A=PBP^{-1}$. Allora esiste $ Q\in\GL\left(n,\ \R\right)$ tale che $A=QBQ^{-1}$.
\end{theorem}
\begin{proof}{n}
Innanzitutto, $A=PBP^{-1}$ se e solo se $AP=PB$. Consideriamo le soluzioni $X$, matrice $n\times n$ a coefficienti reali, del sistema lineare omogeneo in $n^2$ equazioni in $n^2$ incognite $AX=XB$. Sia $W\subseteq \C^{n,n}$ il sottospazio \textit{vettoriale} delle soluzioni (\textit{complesse}) del sistema: sappiamo già che $P\in W$, dunque $W\neq\left\{O\right\}$. Sia allora $k=\dim W\geq 1$ e sia $C_1,\ \ldots,\ C_k\in\C^{k,\ k}$ una base di $W$. Esse sono matrici complesse, dunque possiamo scomporla nella sua parte reale e immaginaria:
\begin{equation*}
\forall j=1,\ \ldots,\ k\quad C_j=X_j+iY_j,\ X_j,\ Y_j\in\R^{n,n}
\end{equation*}
...
\begin{equation*}
	...
	\end{array}
\end{equation*}
Le matrici $AX_j,\ AY_j,\ X_jB$ e $Y_jB$ sono tutte in $\R^{n,n}$. Due matrici complesse scomposte come in precedenza sono uguali se e solo se la parte reale e l'argomento sono uguali,
\begin{equation*}
	AX_j=X_jB\qquad
	AY_j=Y_jB
\end{equation*}
ma allora $X_j,\ Y_j\in W\ \forall j$; poiché $C_1,\ \ldots,\ C_k$ è una base di $W$, e quindi lo generano. Per costruzione $C_j=X_j+iY_j$, dunque anche $X_1,\ \ldots, X_k,\ Y_1,\ \ldots Y_k$ generano $W$ come spazio vettoriale \textit{complesso}. Sicuramente $\left\{X_1,\ \ldots, X_k,\ Y_1,\ \ldots Y_k\right\}$ contiene una base di $W$, cioè $\exists D_1,\ \ldots,\ D_k$ base $\mathcal{D}$ di $W$ con $D_j\in\R^{n,n}\ \forall j$. Dalle condizioni in cui ci siamo posti, la matrice $Q$ cercate deve soddisfare i seguenti requisiti:
\begin{itemize}
	\item $Q\in W$.
	\item $Q\in\R^{n,n}$.
	\item $Q$ invertibile.
...
\begin{equation*}
	D=t_1D_1+\ldots +t_kD_k\quad\text{con }t_1,\ \ldots,\ t_k\in\C^{n,n}
\end{equation*}
