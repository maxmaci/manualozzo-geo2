...
\begin{define}\textsc{Azione di un gruppo su un insieme.}\\
	Sia $G$ un gruppo e $X$ un insieme. Un'\textbf{azione}\index{azione} di $G$ su $X$ è
		\begin{itemize}
...
\begin{define}\textsc{Relazione di equivalenza dell'azione.}\\
...
\begin{demonstration}
	...
	\begin{itemize}
		\item \textsc{Riflessiva}: $x=e\ldotp x$
		\item \textsc{Simmetrica}: $y=g\ldotp x \implies x=g^{-1}\ldotp y$
		\item \textsc{Transitiva}: $y=g\ldotp x,\ z=h\ldotp y \implies z=(hg)\ldotp x$
	\end{itemize}
\vspace{-3mm}
\end{demonstration}
