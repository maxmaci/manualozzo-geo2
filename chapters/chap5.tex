...
\vspace{-3mm}
\end{demonstration}
\begin{define}[Orbita di un elemento.]~{}\\
	Le classi di equivalenza date da questa relazione sono dette \textbf{orbite}\index{orbita}:
		\begin{equation*}
			[x]=O\left(x\right)=G\ldotp x=\{y\in X \mid \exists g\in G \colon y=g\ldotp x\}=\{ g\ldotp x\in X\mid g\in G \}
		\end{equation*}
...
\end{enumerate}
Dunque ne consegue, dal teorema di Lagrange, che $\lvert O\left(x\right)\rvert = \left[G\ \colon H_x\right]= \frac{\lvert G\rvert}{\lvert H_x\rvert}$.
\end{demonstration}
\begin{observe}
...
\end{observe}
\begin{demonstration}~{}\\
$\includedx$Sia $h\in H_{x_2}$. Poiché  per ipotesi $x_2=g\ldotp x_1$, si ha:
\begin{equation*}
...
\begin{equation*}
	\theta_e\left(x\right)=e\ldotp x=x=Id\left(x\right)\quad \forall x
\end{equation*}
...
\end{equation*}
Cioè l'inversa di $\theta_g$ è $\theta_{g^{-1}}$. Ma allora per ipotesi, anche $\theta_{g^{-1}}$ è continua perché $g^{-1}\in G$, dunque segue la tesi.
\end{demonstration}
...
Ogni elemento di $A$ è contenuto in un orbita, dunque $\pi\left(A\right)$ contiene le orbite degli $x\in A$; la controimmagine $\pi^{-1}\left(\pi\left(A\right)\right)$ risulta dunque pari all'unione di \textit{tutte} le orbite in $X$ che intersecano l'insieme $A$, che indichiamo con $g\ldotp A$:
\begin{equation*}
...
\end{equation*}
Definiti gli (intorni) aperti $U\in I\left(x\right)$ e $V\in I\left(x\right)$ disgiunti (in quanto $X$ di \textbf{Hausdorff}), possiamo considerare gli altri (intorni) aperti disgiunti $g_i\ldotp U\in I\left(g_i\ldotp x\right),\ g_i\ldotp V\in I\left(g_i\ldotp y\right)$. Allora:
\begin{equation}
...
\begin{equation*}
	...
	\left(\bigcup_{i}g_i\ldotp U\right)\cap \left(\bigcup_{i}g_i\ldotp V\right)=\emptyset\implies \widetilde{U}\cap \widetilde{V}=\emptyset
\end{equation*}	
\end{itemize}
\end{enumerate}
\vspace{-6mm}
...
\end{example}
\begin{demonstration}
Consideriamo $\funztot{f}{\realset}{S^1\subseteq \realset}{t}{\left(\cos\left(2\pi t\right),\sin\left(2\pi t\right)\right)}$.
\begin{itemize}
\item $f$ è continua.
\item $f$ è suriettiva.
\item $f\left(t_1\right)=f\left(t_2\right)\iff t_1-t_2\in \integerset \iff t_1,\ t_2$ nella stessa orbita$\iff \pi\left(t_1\right)=\pi\left(t_2\right)\iff t_1\sim t_2$
\end{itemize}
