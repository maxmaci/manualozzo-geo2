...
\end{demonstration}
\begin{observe}
Punti nella stessa orbita hanno stabilizzatori \textbf{coniugati}\index{coniugati}\index{stabilizzatore!coniugato}:
\begin{equation}
...
\end{example}
\begin{define}
	Lo \textbf{spazio proiettivo reale}\index{spazio topologico!spazio proiettivo reale} $\mathbb{P}^n\left(\realset\right)$ (o $\realset\mathbb{P}^n$) di dimensione $n$ \index{spazio!proiettivo reale} è lo spazio topologico delle rette vettoriali passanti origine in $\realset^{n+1}$, a cui son state tolte l'origine. È definito come lo spazio quoziente rispetto all'azione del gruppo moltiplicativo $\realset^{\ast}$:
	\begin{equation}
