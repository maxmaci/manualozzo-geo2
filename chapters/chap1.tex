...
\end{flushright}
\end{introduction}
\noindent Immaginiamo di aver un oggetto costituito di un magico \emph{materiale elastico} che possiamo allungare, piegare, torcere e rimpicciolire a piacere, ma che non possiamo né strappare né incollarne parti. Quali proprietà dell'oggetto si mantengono \emph{inalterate} prima e dopo la deformazione?\\
Il principale scopo della \textbf{Topologia} è studiare le proprietà che rimangono invariate da queste deformazioni continue. Per far ciò, è necessario dotare un insieme di una struttura, detta \emph{topologia}, che permetta la definizione di \emph{continuità} di una funzione e di \emph{omeomorfismo}, generalizzando ciò detto in precedenza.\\
In questo capitolo ci occuperemo di introdurre questi concetti fondamentali per poi studiare alcuni risultati che ne conseguono. Inoltre, affronteremo alcuni degli \emph{assiomi di separazione} e la nozione di \emph{metrica}.
...
\begin{define}\textsc{Finezza.}\index{finezza}\\
...
\end{itemize}
%aggiungere citazione all'esempio
\end{examples}
...
Sia $X$ spazio topologico e $A\subseteq X$. A è \textbf{denso}\index{spazio!denso} è denso in $X$ se $\overline{A}=X$ o, in altri termini, tutti i punti di $X$ sono aderenti ad $A$.
\end{define}
\begin{example}
...
\end{demonstration}
\begin{define}\textsc{Funzione aperte e funzione chiusa.}\\
Siano $X$, $Y$ spazi topologici e $\funz{f}{X}{Y}$ funzione.
\begin{itemize}
...
\end{observe}
\begin{attention}
	Una funzione $f$ aperta che non sia omeomorfismo \textit{non} è necessariamente una funzione aperta. Si prenda $\funztot{f}{\realset^2}{\realset}{\left(x,\ y\right)}{x}$ (la proiezione sulla prima coordinata):
	\begin{itemize}
...
\end{itemize}
Allora su $Y$ si può definire la \textbf{topologia indotta}\index{topologia!indotta} come la topologia meno fine tra tutte quelle che rendono $f$ continua.
\end{define}
...
\vspace{-6mm}
\end{observe}
\begin{example} \textsc{Esempio di \textit{non} immersione.}
	\begin{equation}
...
Siano $P,\ Q$ spazi topologici e $P\times Q$ il suo prodotto cartesiano. Definite le \textbf{proiezioni}\index{proiezione!al prodotto cartesiano}:
\begin{gather}
...
\vspace{-6mm}
\end{demonstration}
\begin{proposition}\label{prodottihause}\textsc{(Manetti, 3.6.8)}\\
...
\end{equation}
Allora $C$ è chiuso.
\item  Sia $\funz{f}{X}{X}$ continua, $X$ di \textbf{Hausdorff}.  Sia $F_{ix}\left(f\right)$ il luogo dei \textbf{punti fissi}\index{punto!fisso} di $f$ e $g$ coincidono:
\begin{equation}
...
\end{equation}
\item Sia $\funz{f}{X}{Y}$ continua, $Y$ di \textbf{Hausdorff}. Sia $\Gamma_f$ il \textbf{grafico}\index{grafico} di $f$ le insieme delle coppie $(x,f\left(x\right))$ formate dai punti del dominio e le corrispettive immagini tramite $f$.
\begin{equation}
