...
\begin{observe}~{}
	\begin{itemize}
		...
		\item Definiamo il blocco di Jordan di dimensione $k$ con autovalore zero, necessario per calcolare l'autospazio $V_\lambda$:
		\begin{equation}\setlength\arraycolsep{0.5mm}
			N=J-\lambda I= \left(
				\begin{array}{ccccc}
				0	& 1 		&  0		& \ldots 	& 0 \\
...
La seguente matrice $9\times 9$  è in forma di Jordan, con blocchi $J_3\left(2\right)$, $J_2\left(i\right)$, $J_3\left(i\right)$ e $J_1\left(-4\right)$:
	\begin{equation*}
		...
		\tikz[baseline=(M.west)]{%
			...
			\node[draw,fit=(M-1-1)(M-3-3),inner sep=-0.25pt] {};
			\node[draw,fit=(M-4-4)(M-5-5),inner sep=-0.25pt] {};
			\node[draw,fit=(M-6-6)(M-8-8),inner sep=-0.25pt] {};
			\node[draw,fit=(M-9-9),inner sep=-0.25pt] {};
		}
	\end{equation*}
\end{example}
% https://github.com/MartinThoma/LaTeX-examples/blob/master/tikz/jordan-normal-form-block/jordan-normal-form-block.tex (check it out)
