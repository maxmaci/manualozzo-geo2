...
\begin{example}~{$\nicefrac{D^n}{\sim}\cong S^n$}\\
	...
	\begin{equation*}
		\funztot f {D^n} {S^n} x {\left( 2x\sqrt{1- \left| x \right|^2}, \ 2\left| x\right|^2 -1 \right)}
	\end{equation*}
	Dunque $\nicefrac{D^n}{\sim}\cong S^n$ per la relazione:
\begin{equation*}
	\underline{x}\sim \underline{y} \iff \underline{x}=\underline{y} \vee \labs \underline{x} \rabs ^2 = 1 = \labs \underline{y} \rabs ^2
\end{equation*}
...
\begin{intuit}
	...
	Lo spazio quoziente ottenuto è $S^2$: possiamo ottenerlo visualmente ‘‘gonfiando'' l'interno del disco per poi chiudere il ‘‘palloncino'' ottenuto lungo i punti del bordo, come in figura.	%inserire immagine sfera
\end{intuit}
...
\begin{example} $\nicefrac{D^n}{S^{n-1}}\cong S^n$ \\
	Cerchiamo ora di generalizzare l'esempio precedente. Ricordiamo che relazione c'è fra i dischi e le sfere:
		\begin{gather*}
			D^n=\text{ disco in } \realset^n=\{x\in\realset^n \mid \left| x \right| \leq 1 \}\\
			S^{n-1}=\text{ bordo di } D^n=\{x\in\realset^n \mid \left| x \right| = 1 \}
		\end{gather*}
...
\begin{example} \textsc{Cono su} $S^n \cong D^{n+1}$.\\
	...
	%AAA DISEGNI CERCASI
	n generale, considerata la funzione:
	\begin{equation*}
	...
	Verifichiamo che la relazione di equivalenza indotta da $f$ è proprio quella di contrazione:
		\begin{gather*}
			(\mathbf{x}, \ t)\sim (\mathbf{y}, \ s) \iff f(\mathbf{x}, \ t)=f(\mathbf{y}, \ s) \iff t\mathbf{x}=s\mathbf{y} \iff \begin{cases} 
				...
				t=s=0 
			\end{cases}\\
			\text{Se } t\neq 0 \implies \mathbf{x}=\frac{s}{t}\mathbf{y}, \text{ ma } \labs\mathbf{x}\rabs=1 \implies \lvert \frac{s}{t} \rvert \underbrace{\labs\mathbf{y}\rabs}_{=1}=1 \implies \lvert \frac{s}{t} \rvert=1 \implies s=t \implies \mathbf{x=y}
		\end{gather*}
	\vspace{-6mm}
\end{example}
\begin{intuit}
...
\begin{demonstration}
	...
	$3\implies 2)$ Per dimostrare che $f$ è chiusa bisogna far vedere che $\forall C\subseteq X, C$ chiuso $\implies f(C)\subseteq Y$ chiuso, ma $f$ è identificazione $\iff f^{-1}\left( f(C) \right)\subseteq X$ chiuso. Notiamo che $f^{-1}\left( f(C) \right)= p_1(k\cap p_2^{-1}(C))$, infatti 
		...
		\end{gather*}
	$2\implies 1)$ Serve il teorema di Wallace, pertanto non ne affronteremo la dimostrazione.
\end{demonstration}