...
\begin{demonstration}
	\begin{enumerate}[label=\Roman*]
		...
		\end{cases}
		\end{equation*}
	\item Consideriamo $\funz{f}{\realset^n}{Y=\left\{1\text{ punto}\right\}}$ e $\funz{g}{Y=\left\{1\text{ punto}\right\}}{\realset^n}$, ponendo $g\left(\text{punto}\right)=\underline{0}$.\\
...
\end{demonstration}
\begin{define}
	Una spazio topologico è \textbf{contraibile}\index{contraibile} o \textit{contrattile}\seeonlyindex{contrattile}{contraibile} se hanno lo stesso tipo di omotopia di un punto.
\end{define}
\begin{examples}~{}
	\begin{enumerate}
...
\begin{example}\textsc{La retrazione radiale.}\label{retrazioneradiale}\\
	Sia $X=\realset^n\setminus \left\{\underline{0}\right\}$ e $A=S^{n-1}\subseteq X$. Vogliamo definire una retrazione di $X$ su $A$, cioè una funzione continua $\funz{r}{\realset^n\setminus\left\{\underline{0}\right\}}{A=S^{n-1}}$ tale che $r_{\mid S^{n-1}}=Id_{S^{n-1}}$. Definiamo allora la \textbf{retrazione radiale}\index{retrazione radiale}:
	\begin{equation}
		\funztot{r}{\realset^n\setminus\left\{\underline{0}\right\}}{S^{n-1}}{\underline{x}}{r\left(\underline{x}\right)=\frac{\underline{x}}{\lvert\lvert \underline{x}\rvert\rvert}}
	\end{equation}
\begin{itemize}
	\item $r$ è ben definita perché $\underline{x}\neq\underline{0}\implies \lvert\lvert \underline{x}\rvert\rvert\neq 0$.
	\item $r$ continua.
	\item Se $\underline{x}\in S^{n-1}$, allora $\lvert\lvert \underline{x}\rvert\rvert=1$, cioè $\forall \underline{x}\in S^{n-1}\ r\left(\underline{x}\right)=\frac{\underline{x}}{\lvert\lvert \underline{x}\rvert\rvert}=\underline{x}$.
\end{itemize}
\end{example}
\begin{define}
...
\end{equation*}
	\begin{equation*}
		\funztot{f}{X}{X}{\underline{x}}{\frac{\underline{x}}{\lvert\lvert \underline{x}\rvert\rvert}}
	%\funz{\tilde{r}\coloneqq i\circ r}{\realset^n\setminus\left\{\underline{0}\right\}}{\realset^n\setminus\left\{\underline{0}\right\}}{\underline{x}}{\frac{\underline{x}}{\lvert\lvert \underline{x}\rvert\rvert}}
\end{equation*}
Vogliamo che $\tilde{r}$ sia omotopa a $Id_X$.
Osserviamo che $\forall \underline{x}\in\realset^n\setminus\left\{\underline{0}\right\}=X$ il segmento da $\underline{x}$ a $\frac{\underline{x}}{\lvert\lvert \underline{x}\rvert\rvert}$ non contiene, per costruzione, l'origine: allora esso è interamente contenuto in $\realset^n\setminus \left\{\underline{0}\right\}=X$. Dunque, riprendendo l'osservazione di pag. \pageref{omotopiasegmento} definiamo l'omotopia:
\begin{equation*}
	\funztot{F}{X\times I}{X}{\left(\underline{x},\ t\right)}{\left(1-t\right)\underline{x}+t\frac{\underline{x}}{\lvert\lvert \underline{x}\rvert\rvert}}
\end{equation*}
Infatti $F$ è ben definita, continua e $\mvf{F}{\underline{x}}{0}=\frac{\underline{x}}{\lvert\lvert \underline{x}\rvert\rvert}=\tilde{r}\left(\overline{x}\right),\ \mvf{F}{x}{1}=\overline{x}=Id_X\left(\overline{x}\right)$.
\end{example}
\begin{corollary}
...
\begin{examples}\textsc{Altri esempi di equivalenze omotopiche}
	\begin{enumerate}
		\item $\realset^2\setminus\left\{2\text{ punti}\right\}$ ha lo stesso tipo di omotopia di un bouquet di due circonferenze: si può ottenere attraverso una composizione (continua) di omotopie radiali e lineari.
		\item $\realset^2\setminus\left\{n\text{ punti}\right\}$ ha lo stesso tipo di omotopia di un bouquet di $n$ circonferenze.
		\item $\realset^3\setminus\left\{1\text{ retta}\right\}$ ha lo stesso tipo di omotopia di $\realset^2\setminus\left\{1\text{ punto}\right\}$ per omotopie lineari, dunque ha la stessa omotopia di $S^1$ per i ragionamenti precedenti.
	\end{enumerate}
\end{examples}