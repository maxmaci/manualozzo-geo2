...
\begin{define} \textsc{Punto singolare} \index{punto!singolare}
...
\begin{theorema} \textsc{Relazione di Eulero} \index{Eulero!relazione} \\
	Sia $F\in\kamp[x_0,\dots,x_n]$ polinomio omogeneo di grado $m$ e le sue derivate parziali $\frac{\partial{f}}{\partial{x_i}}\in\kamp[x_0,\dots,x_n]$, polinomio omogenei di grado $m-1$. Si ha che 
		\begin{equation}
...
\end{theorema}
\begin{demonstration}
	Basta mostrarlo per un monomio $G=\lambda x_0^{j_0}\cdots x_n^{j_n}$ con $j_0+\dots+j_n=m,\ \lambda\in\kamp$. Scrivendone le derivate parziali rispetto ad una variabile e poi moltiplicando per $x_i$ sistemiamo l'esponente $i$-esimo
		\begin{gather*}
			\frac{\partial{G}}{\partial{x_i}}=\lambda j_i x_0^{j_0}\cdots x_i^{j_{i-1}} \cdots x_n^{j_n} \implies  x_i \frac{\partial{G}}{\partial{x_i}}=\lambda j_i x_0^{j_0}\cdots x_i^{j_i} x_n^{j_n}\\
			\implies \sum_{i=0}^n x_i\frac{\partial{G}}{\partial{x_i}} = \sum_{i=0}^n \lambda j_i _0^{j_0} \codots x_n^{j_n}=\lambda x_0^{j_0}\cdots x_n^{j_n} \cdot \sum_{i=0}^n j_i =mG
		\end{gather*}
\end{demonstration}
