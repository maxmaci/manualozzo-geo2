...
Una conica è data da un polinomio $F$ omogeneo di grado 2 in $x_0,x_1,x_2$ a coefficienti reali o complessi.
	...
	\end{equation*}
Tale scrittura ci ricorda l'espressione in coordinate una forma quadratica sullo spazio vettoriale $\kamp^3$, e in generale i polinomi omogenei di grado 2 sono forme quadratiche, che abbiamo già affrontato nel corso di Geometria 1.
...
Viceversa, date due coniche $C_1$ e $C_2$ le cui matrici associate (simmetriche) $A_1$ e $A_2$ sono congruenti, allora $C_1$ e $C_2$ sono proiettivamente equivalenti, questo perché vale anche per le forme quadratiche: forme congruenti differiscono per cambiamento di base, ottenendo così la seguente corrispondenza:
	\begin{array}{ccc}
		$C_1$ e $C_2$ equivalenti & \iff & $A_1$ e $A_2$ congruenti
	\end{array}
...
\begin{theorema}
	\begin{enumerate}
		...
		\item	forma canonica: ogni conica di P2C è proiettiamente equivalente ad una ed una sola delle tre coniche seguenti\\
			\begin{array}{ccc}
...
\begin{theorema}
	\begin{enumerate}
		...
		\begin{array}{cccc}
			$\rk 3\colon$ &	$(3,0)/(0,3)\colon$ & $x_0^2+x_1^2+x_2^2=0$	& \text{conica irriducibile senza punti reali (supporto vuoto)}\\
			%ha un supporto? ci sono dei punti in P2R che hanno questa equazione? no perché escludiamo l'origine	conica irriducibile senza punti reali /tipo ellisse immaginario/ dunque il supporto è vuoto
			& $(1,2)/(2,1) \colon$& $x_0^2+x_1^2-x_2^2=0$ &	\text{conica irriducibile a punti reali, contiene infiniti punti in $\proj[2]{\realset}$}\\
			$\rk 2 \colon$ & $(2,0)/(0,2) \colon$ & $x_0^2+x_1^2=0$ & \text{coppia di rette non reali con 1 punto reale $(0\colon 0\colon 1)$\\
%				perché si fattorizza ma a coeff complessi. Ha un unico punto reale (0\colon 0\colon 1) intersezione delle due rette complesse coniugate
			& $(1,1)\colon $ & $x_0^2-x_1^2=(x_0-x_1)(x_0+x_1)=0$ &	\text{coppia di rette reali distinte}\\ %che si fattorizza su R
			$\rk 1 \colon$ & $(1,0)\colon$ & $x_0^2=0$ & \text{retta doppia}
		\end{array}
	\end{enumerate}
\end{theorema}		
